    ...
    while Count < (Reps):
        ...
        pop.evolve(
            ...
            postOps = [
                ...
                # Only use these if want to terminate simulations when fixation/loss of allele is reach
                #simuPOP.TerminateIf('len(alleleFreq[0]) == 1', stopAll=True),
                #simuPOP.TerminateIf('len(alleleFreq[0]) == 0', stopAll=True),
...
simuFluctuatingSelectionWF(PopSize=1000, Mutation=0.00000005, Generations=1200, NumChrom=1, NumLoci=1, Ploidy=1, Repetitions=6)
